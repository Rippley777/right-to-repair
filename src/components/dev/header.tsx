...
const HomeHeaderSection = () => {
  ...
  const siteAlert = {
    message:
      "Site is currently in beta. Please expect issues. Click to contribute",
    type: "info",
    url: "https://www.github.com/Rippley777/right-to-repair",
  };
  return (
    <>
      {siteAlert && (
        <div
          className={`bg-[#1E3A8A] text-white text-lg font-bold leading-normal tracking-[0.015em] px-10 py-3`}
          onClick={siteAlert.url ? () => window.open(siteAlert.url) : () => {}}
        >
          {siteAlert.message}
        </div>
      )}
